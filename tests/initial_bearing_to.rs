...
fn initial_bearing_to_same_position() {
    let p = HorizontalPos::from_s84(50.066389, -179.999722);
    assert_eq!(Err(Error::CoincidentalPositions), p.initial_bearing_to(p));
    assert_eq!(
        Err(Error::CoincidentalPositions),
        p.initial_bearing_to(HorizontalPos::from_s84(50.066389, -179.999722))
    );
}
...
fn at_prime_meridian_initial_bearing_to_north() {
    let p1 = HorizontalPos::from_s84(50.0, 0.0);
    let p2 = HorizontalPos::from_s84(58.0, 0.0);
    assert_eq!(Ok(Angle::zero()), p1.initial_bearing_to(p2));
}

#[test]
fn at_prime_meridian_initial_bearing_to_south() {
    let p1 = HorizontalPos::from_s84(58.0, 0.0);
    let p2 = HorizontalPos::from_s84(50.0, 0.0);
    assert_eq!(
        Ok(Angle::from_decimal_degrees(180.0)),
        p1.initial_bearing_to(p2)
    );
...
fn at_date_line_initial_bearing_to_north() {
    let p1 = HorizontalPos::from_s84(50.0, 180.0);
    let p2 = HorizontalPos::from_s84(58.0, 180.0);
    assert_eq!(Ok(Angle::zero()), p1.initial_bearing_to(p2));
}

#[test]
fn at_date_line_initial_bearing_to_south() {
    let p1 = HorizontalPos::from_s84(58.0, 180.0);
...
fn south_pole_initial_bearing_to_north_pole() {
    ...
    let p2 = HorizontalPos::north_pole(S84);
    assert_eq!(Ok(Angle::zero()), p1.initial_bearing_to(p2));
}
...
fn north_pole_initial_bearing_to_south_pole() {
    ...
    let p2 = HorizontalPos::south_pole(S84);
    assert_eq!(Ok(Angle::zero()), p1.initial_bearing_to(p2));
}

#[test]
fn north_pole_initial_bearing_to_date_line() {
    let p1 = HorizontalPos::north_pole(S84);
    let p2 = HorizontalPos::from_s84(50.0, 180.0);
    assert_eq!(
...
fn initial_bearing_to() {
    ...
    assert_eq!(
        Angle::from_decimal_degrees(9.1198173275),
        p1.initial_bearing_to(p2).unwrap().round(Microarcsecond)
    );
    assert_eq!(
        Angle::from_decimal_degrees(191.27520031611112),
        p2.initial_bearing_to(p1).unwrap().round(Microarcsecond)
    );
}
